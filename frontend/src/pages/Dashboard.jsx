...
export default function Dashboard() {
  ...
  if (!dashboard || !overview || overview.total_questions_solved === 0) {
    return (
      <Card className="text-center py-12">
        <Zap className="w-12 h-12 mx-auto mb-4 text-accent-500" />
        <h2 className="text-xl font-bold mb-2">Welcome to BYOP Studio — IBPS SO</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-md mx-auto">
          Import questions, log an external mock, or start your first practice session.
        </p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-mono font-semibold">{daysLeft}d</span>
          <span className="text-gray-500">until exam</span>
        </div>
      </div>
      ...
      {/* Section Readiness with expandable syllabus panel */}
      {dashboard.sections?.length > 0 && (
        <Card>
          ...
          <div className="mt-3 space-y-2">
            ...
            {/* Non-PK sections */}
            {dashboard.sections
              .filter((s) => SECTIONS[s.section]?.type === "non_pk")
              .map((s) => (
                <SectionRow
                  key={s.section}
                  section={s}
                  isExpanded={expandedSection === s.section}
                  onToggle={() => setExpandedSection(expandedSection === s.section ? null : s.section)}
                />
              ))}

            {/* PK sections */}
            {dashboard.sections
              .filter((s) => SECTIONS[s.section]?.type === "pk")
              .map((s) => (
                <SectionRow
                  key={s.section}
                  section={s}
          ...
          {/* Total penalty */}
          {dashboard.total_penalty_this_month > 0 && (
            <div className="mt-3 text-xs text-red-500 font-mono text-right">
              Total penalty this month: −{dashboard.total_penalty_this_month.toFixed(2)} marks
            </div>
          )}
        </Card>
      )}
      ...
      {/* Recent mocks */}
      {dashboard.recent_mocks?.length > 0 && (
        <Card>
